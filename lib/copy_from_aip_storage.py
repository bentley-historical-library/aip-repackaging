...
def identify_aips_to_copy(uuids, to_do_dir):
    copy_list = []
    for uuid in uuids:
        if not os.path.exists(os.path.join(to_do_dir, uuid)):
            copy_list.append(uuid)
    return copy_list
...
def copy_from_aip_storage(AIPRepackager):
    ...
    uuids_to_copy = identify_aips_to_copy(AIPRepackager.project_metadata["uuids"], to_do_dir)
    for uuid in tqdm(uuids_to_copy, desc="Copying AIPs"):
        ...
        if os.path.exists(uuid_base_dirpath):
            print("COPYING {} TO {}".format(uuid_base_dirpath, to_do_dir))
            ...
            tqdm.write("copied {}".format(uuid))

    for uuid in tqdm(uuids_to_copy, desc="Moving AIPs"):
        ...
        if os.path.exists(path_to_aip) and not os.path.exists(path_to_dst):
            ...
            tqdm.write("moved {}".format(aip_name))
